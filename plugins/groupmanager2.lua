      ...
      if data[tostring(msg.to.id)] then
        ...
        elseif matches[1] == 'link' then
          ...
          -- unlock {bot|name|member|photo|sticker}
		    ...
		    elseif matches[3] == 'english' and is_mod(msg.from.id, msg.to.id) then
			    if settings.lock_english == 'no' then
                return 'group english is not locked'
                else
                settings.lock_english = 'no'	
                save_data(_config.moderation.data, data)	
          ...
          elseif matches[2] == 'settings' and is_mod(msg.from.id, msg.to.id) then
            ...
            if settings.lock_photo == 'yes' then
              lock_photo_state = 'yes'
            elseif settings.lock_photo == 'no' then
              lock_photo_state = 'no'
            end
			if settings.lock_english == 'yes' then
              lock_english_state = 'yes'
            elseif settings.lock_english == 'no' then
              lock_english_state = 'no'
            end
            if settings.lock_member == 'yes' then
              lock_member_state = 'yes'
            elseif settings.lock_member == 'no' then
              lock_member_state = 'no'
            end
            ...
            local text = 'Group settings:\n'
				  ...
				  ..'\n'..lock_english_state..' Lock group english : '..settings.lock_english
        ...
        elseif msg.media and msg.media.caption == 'sticker.webp' and not is_sudo(msg.from.id) then
          ...
          elseif settings.sticker == 'ok' then
            return nil
		     ...
		     if settings.english == 'yes' then
            chat_del_user(get_receiver(msg), 'user#id'..user_id, ok_cb, true)
            return 'Dont speak english'
          elseif settings.english == 'no' then
            return nil
          end
  ...
  return {
    ...
    usage = {
      ...
      moderator = {
		...
		'!group <lock|unlock> english : lock/unlock group english.',
