  ...
  local function run(msg, matches)
    ...
    if is_sudo(msg.from.id) then
      ...
      elseif matches[1] == 'disable' then
        ...
        local k = plug_enabled(matches[2])
        -- Check if plug is enabled
        if not k then
          return 'Plug '..matches[2]..' not enabled'
        end
        -- Disable and reload
  ...
  return {
    ...
    usage = {
      moderator = {
        ...
        '!plug en [plugin] chat: re-enable plugin only this chat.',
        '!plug di [plugin] chat: disable plugin only this chat.'
      },
      sudo = {
        '!plug en [plugin]: enable plugin.',
        '!plug di [plugin]: disable plugin.',
